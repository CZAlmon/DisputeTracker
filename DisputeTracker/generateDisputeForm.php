...
if ($_SERVER["REQUEST_METHOD"] == "POST"){
	...
	if(isset($_POST["redoDisputeForm"])){
		
		$caseID = $_POST["redoDisputeForm"];
		
		$CaseDONEQuery = "SELECT casedoneinput FROM checkcases where id='" . $caseID . "'";
		...
		if($CaseDONEData[0][0] == '0'){
			$_SESSION["ADD_DISPUTE_ERROR"] = 'Case Number: '.$caseID.' is not done! You can\'t make dispute forms until the case is done. Contact IT if you have any questions.';
			header( "Location: index.php" );
			exit();
		}
		...
		//var_dump($caseID, $AccountNumData);
		//exit();
		...
		$transactionsINFO_query = "SELECT id, cardid, amount, transactiondate, dateposted, disputereason, description, merchantname, merchantcontacted, merchantcontacteddate, merchantcontactdescription, receiptstatus FROM checktransactions WHERE transactiondeleted=FALSE AND caseid='" . $caseID . "'";
		...
		$_SESSION["ADD_DISPUTE_ERROR"] = 'Something went wrong with the Server POST. Please try again.';
		header( "Location: index.php" );
		exit();
	}
...
if($accesslevel < 3){
	$_SESSION["TransactionAdded"] = 'The VISA Dispute Form is being remade. Here is the link: <a href="'.$VISACheckcardURL.'">Click me for the Dispute Form</a>. Check the <a href="viewGeneratedDocuments.php">View Generated Documents</a> page again if it doesn\'t work.';
	header( "Location: DisputeViewOnly.php" );
	exit();
}
else{
	$_SESSION["TransactionAdded"] = 'The VISA Dispute Form is being remade. Here is the link: <a href="'.$VISACheckcardURL.'">Click me for the Dispute Form</a>. Check the <a href="viewGeneratedDocuments.php">View Generated Documents</a> page again if it doesn\'t work.';
...
<head>
    ...
    <title>Dispute Tracker - Dispute Form</title>
	<!-- BOOTSTRAP STYLES-->
...
$PageTitle = "Dispute Track - Generate Forms";
